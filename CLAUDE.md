...
- Keep repeated display labels, status classes, CLP/date formatting, distance, and address helpers in `apps/front/src/lib/display.ts`; do not copy local maps into routes.
- Put repeated mutation invalidation flows in concrete hooks under `apps/front/src/hooks`; avoid generic repository/service layers over `api`.
- Route files can own layout, navigation, and page-specific state, but move request draft defaults, validation, and payload mapping into plain helpers when they grow beyond UI orchestration.
...
- This applies to every piece of UI, not just obvious "forms": any bordered/padded box is `Card`+`CardContent`, not a styled `div`; any clickable element is `Button` (`asChild` for link-as-button, `data-icon="inline-start"` on a leading icon), not a styled `<button>`; any input that gets validated or submitted — including a single OTP/code field — is `useForm` + `Field`/`FieldError` from TanStack Form, not local `useState`. Do not reinvent what shadcn/TanStack already solve.
- **Data tables must use TanStack Table + shadcn Table.** Use `useReactTable` from `@tanstack/react-table` in manual mode (`manualPagination`/`manualSorting`/`manualFiltering`) with server-driven data; URL search params are the single source of truth for table state. See `apps/front/src/routes/_app/available/index.tsx` for the reference pattern.
- **Never filter, sort, or paginate lists client-side** when the backend can do it. Add the query param to the backend route and key the query with it.
- **No hardcoded hex colors in components.** Use the theme tokens defined in `apps/front/src/index.css` (`foreground`, `muted-foreground`, `border`, `muted`, `accent`, plus `ink-soft`/`ink-muted`/`ink-faint`, `surface`/`surface-dim`, and the `panel-*` family for dark marketing panels). Add new tokens to the `@theme` block instead of inlining hexes; inline `style` colors use `var(--color-*)`.
