...
const DEFAULT_FROM = "Gonexo <onboarding@resend.dev>";
...
export async function sendEmail(env: Bindings, to: string, content: EmailContent) {
...
function button(href: string, label: string) {
  return `<a href="${href}" style="display:inline-block;background:#0c8c5e;color:#ffffff;font-weight:600;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none;margin-top:16px">${label}</a>`;
}

function routeLine(origin: string, dest: string) {
  return `<p style="font-size:14px;color:#485450;margin:12px 0 0">${origin}<br/>→ ${dest}</p>`;
}
...
export function newQuoteEmail(p: {
  ...
  return {
    ...
    html: layout(`
      <h1 style="font-size:18px;margin:0">Hola ${p.clientName}, recibiste una cotización</h1>
...
export function quoteAcceptedEmail(p: {
  ...
  return {
    ...
    html: layout(`
      <h1 style="font-size:18px;margin:0">¡Felicitaciones ${p.driverName}!</h1>
      <p style="font-size:14px;color:#485450;margin:12px 0 0">
...
export function confirmReminderEmail(p: {
  ...
  return {
    ...
    html: layout(`
      <h1 style="font-size:18px;margin:0">Hola ${p.clientName}, tu flete fue marcado como entregado</h1>
      <p style="font-size:14px;color:#485450;margin:12px 0 0">
