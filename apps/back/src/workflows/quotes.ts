...
export async function acceptQuote(db: Db, userId: string, quoteId: string) {
  ...
  try {
    ...
    db.insert(job).select(db.select({
      ...
      mercadoPagoPreferenceId: sql<string | null>`null`.as("mercado_pago_preference_id"),
      mercadoPagoPaymentId: sql<string | null>`null`.as("mercado_pago_payment_id"),
      paidAt: sql<Date | null>`null`.as("paid_at"),
      driverSettledAt: sql<Date | null>`null`.as("driver_settled_at"),
      driverSettlementReference: sql<string | null>`null`.as("driver_settlement_reference"),
